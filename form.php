...
// Ensure the session stage is set and does not exceed 5
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['previous'])) {
        // Move to the previous stage but never below Stage 1
        ...
        foreach ($_POST['answers'] as $question_id => $answer) {
            ...
            $stmt->bind_param("siss", $_SESSION['email'], $question_id, $answer, $answer);
            $stmt->execute();
        }

        // Move to next stage but never exceed Stage 5
        if ($_SESSION['stage'] < 5) {
            $_SESSION['stage']++;
        } else {
            header("Location: result.php"); // Redirect to final submission
            exit();
        }
    }

    // Redirect to refresh session values
...
<html lang="en-US" dir="ltr">
  ...
  <head>
    ...
    <!--    Document Title-->
    <title>Stage <?php echo $current_stage; ?> Form</title>
      ...
      <section class="pt-5 mb-6" id="feature">
        <div class="container mt-5">
            ...
            <form method="post">
                ...
                <?php endforeach; ?>
                <br>
                <button type="submit" name="previous" <?php echo ($current_stage == 1) ? 'disabled' : ''; ?>>Previous Stage</button>
                <button type="submit"><?php echo ($current_stage == 5) ? 'Submit Final' : 'Next Stage'; ?></button>
            </form>
